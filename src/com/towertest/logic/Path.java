...
/**
 * Path is a list of waypoints used to travel The ArrayList waypoints is public
...
public class Path implements Cloneable {
	...
	/**
	 * initialized the list and finds a path based on current level
	 * configuration
	 */
	public Path(Enemy en, Waypoint pEnd, TMXLayer pTmxlayer, GameMap map) {
		enemy = en;
		...
		if (findPath()) {
			...
			// xyPath = new
			// org.andengine.entity.modifier.PathModifier.Path(A_Path.getLength());
			convertRCPathtoXY();
			optimizeXYPath();
		}
		// xyPath = xyPath.to(TowerTest.getXFromCol(A_Path.getX(i)),
		// TowerTest.getYFromRow(A_Path.getY(i)));
	}

	public Path(Enemy en, Waypoint pEnd, TMXLayer pTmxlayer, GameMap map,
			ArrayList<Point> pxyPath,
	...
	IPathFinderMap<Enemy> PathMap = new IPathFinderMap<Enemy>() {
		...
		public boolean isBlocked(int pX, int pY, Enemy pEntity) {
			try {
				...
				for (int i = 0; i < map.getStartLoc().length; i++) {
					if ((pX == map.getStartLoc()[i].x)
							&& (pY == map.getStartLoc()[i].y)) {
						return false;
					}
				}
				return true;
	...
	ICostFunction<Enemy> CostCallback = new ICostFunction<Enemy>() {
		@Override
		public float getCost(IPathFinderMap<Enemy> pPathFinderMap, int pFromX,
				int pFromY, int pToX, int pToY, Enemy pEntity) {
	...
	public boolean checkRemainingPath(int pX, int pY) {
		if (rcPath != null) {
			...
			final int enCol = Utils.getColFromX(enemy.getX());
			final int enRow = Utils.getRowFromY(enemy.getY());
			for (int i = len - 1; i >= 0; i--) { // starts checking from the end
				if (xs[i] == pX && ys[i] == pY) {
					...
					return false; // we found the enemy first, so no reason to
									// check the rest
				}
			}
		} else {
			return false; // no reason to check the path if it's crapola right?
							// :-P
		}
	...
	private boolean findPath() {
		// coords
		try {
			AStarPathFinder<Enemy> finder = new AStarPathFinder<Enemy>();
			rcPath = finder.findPath(PathMap, -1, -1,
					tmxlayer.getTileColumns(), tmxlayer.getTileRows(), enemy,
					enemy.getCol(), enemy.getRow(), end.x, end.y, false,
		 ...
		 * far the bullet can go, in this case up to the enemy float dX =
		 * target.getMidX() -
		 * this.getMidX();//+(Math.abs(gY)/Enemy.speed/Projectile.speed); float
		 * dist = (float) Math.sqrt(Math.pow(dX, 2) + Math.pow(dY, 2)); //D=r*t
		 * //therefore t = D/r trajectory = new
		 * MoveByModifier(dist/Projectile.speed, dX, dY);
	...
	private void convertRCPathtoXY() {
		...
		for (int i = 0; i < rcPath.getLength(); i++)
			xyPath.add(new Point(Utils.getXFromCol(rcPath.getX(i)), Utils
					.getYFromRow(rcPath.getY(i))));
		xyPath.add(0, new Point(enemy.getX(), enemy.getY())); // add their
																// current
																// location to
																// the beginning
	}
	...
	/**
	 * ensures that there are no overlapping paths, as well as removes
	 * middle-nodes for straight paths
	 */
	private void optimizeXYPath() {
		...
		if (xyPath.size() > 2) { // no point in doing it if the line is already
									// as short as it can be
			// set the starting angle to the first two points
			currentAngle = Math.atan2(xyPath.get(1).y - xyPath.get(0).y,
					xyPath.get(1).x - xyPath.get(0).x); // -pi to pi
			// currentAngle = ((currentAngle < 0) ? currentAngle + 2 * Math.PI :
			// currentAngle); //make it positive
			for (int i = 1; i < xyPath.size() - 1; i++) { // flip through all
															// the points to see
															// if we should
															// remove the
															// previous
				// if it's on the same angle as our last, then remove the
				// previous
				// what's our angle from the last point
				newAngle = Math.atan2(xyPath.get(i + 1).y - xyPath.get(i).y,
						xyPath.get(i + 1).x - xyPath.get(i).x);
				newAngleInverse = ((newAngle > 0) ? newAngle - Math.PI
						: newAngle + Math.PI); // the inverse angle, allows it
												// to treat angles
				// that are the opposite direction as the same direction,
				// therefore removing double-backs :-)
				if ((Math.abs(newAngle - currentAngle) < tolerance)
						|| (Math.abs(newAngleInverse - currentAngle) < tolerance)) { // onARoll
																						// and
																						// the
																						// angles
																						// are
																						...
																						// (close
																						// enough)
					xyPath.remove(i);// remove the previous point, since it's
										// the same angle as our current
					i--;
				} else {
					// we must have changed direction, so store this current
					// point as our new corner
	...
	public double getCurrentAngle() {
		...
		double b;
		if (xyPath.size() > 1) {
			for (int i = 0; i < xyPath.size() - 1; i++) { // flip through all
															// the segments
				if (((xyPath.get(i).y <= enemy.getY()) && (enemy.getY() <= xyPath
						.get(i + 1).y))
						|| ((xyPath.get(i + 1).y <= enemy.getY()) && (enemy
								.getY() <= xyPath.get(i).y))) { // getY() is
																// between y1
																// and y2
					if (((xyPath.get(i).x <= enemy.getX()) && (enemy.getX() <= xyPath
							.get(i + 1).x)) // getX() is between x1 and x2
							|| ((xyPath.get(i + 1).x <= enemy.getX()) && (enemy
									.getX() <= xyPath.get(i).x))) {
						// that means it could be on our line (it's within our
						// box of x1,y1, and x2,y2 all that's left is to check
						// the equation of the line
						// y = m * x + b
						// b = y1 - mx1
						if ((xyPath.get(i + 1).x - xyPath.get(i).x) == 0) { // vertical
																			// line,
																			// therefore
							if (Math.abs(enemy.getX() - xyPath.get(i).x) < tolerance) {
								return Math.atan2(
										xyPath.get(i + 1).y - xyPath.get(i).y,
										xyPath.get(i + 1).x - xyPath.get(i).x);
							}
						} else {
							m = (xyPath.get(i + 1).y - xyPath.get(i).y)
									/ (xyPath.get(i + 1).x - xyPath.get(i).x);
	...
	public Path clone(Enemy newEnemy) {
		if (rcPath == null) {
			Log.d("Path", "rcpath == null");
			return null;
		} else {
			org.andengine.util.algorithm.path.Path tempA_Path = new org.andengine.util.algorithm.path.Path(
					rcPath.getLength());
			for (int i = 0; i < rcPath.getLength(); i++)
			...
			return new Path(newEnemy, end, tmxlayer, map, tempXYPath,
					tempA_Path);
		}
	}
}